// Maximize Video — Chrome/Edge extension (Manifest V3) conversion
// Converted from: https://greasyfork.org/zh-CN/scripts/4870-maximize-video
// Original author: 冻猫 ( icycat )
// Original license: none stated
// Changes vs original userscript:
//   1. Removed // ==UserScript== ... // ==/UserScript== metadata block.
//   2. Replaced custom element name "tbdiv" with "div" (4 occurrences) to avoid
//      relying on an unregistered custom element name in a content script.
//   3. Everything else retained as-is from the original v12.0 userscript.
...
;(() => {
  ...
  const setButton = {
    ...
    locate() {
      ...
      gv.controlBtn.style.left =
        playerRect.screenX - 64 + gv.player.offsetWidth + "px";
      ...
      gv.picinpicBtn.style.left =
        playerRect.screenX - 64 + gv.player.offsetWidth - 54 + "px";
    },
  };
  ...
  const handle = {
    ...
    autoCheck(v) {
      ...
      while ((el = el.parentNode)) {
        if (
          Math.abs(v.offsetWidth - el.offsetWidth) < 15 &&
          Math.abs(v.offsetHeight - el.offsetHeight) < 15
        ) {
          tempPlayer = el;
  ...
  const maximize = {
    playerControl() {
      ...
      if (!gv.isFull) {
        ...
        if (
          gv.autoCheckCount > 0 &&
          !tool.isHalfFullClient(gv.playerChilds[0])
        ) {
